...
if File.exist?("#{$JbossDeploymentsLocation}\\#{$Axis2}.war")	
	...
	execute 'change-folder-backup' do
	  command "ren \"#{$HomeDeployFolder}\\_backups_WS\\temp\" \"Backup-WS-Life%date:/=-%-%time::=-%\""
	end
...
powershell_script 'config_xml_webservice' do
  ...
  code <<-EOH  
  7z d #{$HomeDeployFolder}/ws/#{$Axis2}.war WEB-INF/services/LifeServices.aar
  7z u #{$HomeDeployFolder}/ws/#{$Axis2}.war WEB-INF/services/LifeServices.aar
  EOH
  end  
...
######################### Builds Folder #############################################
	...
	set day=%date:~7,2%
		...
		ren "#{$HomeDeployFolder}\\_builds_WS\\temp" "Builds-WS-Life-%month%-%day%-%year%-%time::=-%"
	EOH
	end
