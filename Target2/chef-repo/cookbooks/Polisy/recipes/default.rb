...
#
######################### Read Config ##############################################	
$lines = File.read('G:\integral\app\pnc\deployv2\config\webconfig2.txt').split("\n")
...
# service "#{$AdminWindowServiceName}" do
	...
	execute  'get_newBuild' do
	  command "#{$HomeDeployFolder}\\..\\scripts\\download-polisy-package.bat"
	  action :run
	end
	
	########################stop admin and batch##############
	...
	powershell_script "stop-#{$BatchJobTaskName}" do
		code <<-EOH
			$a =  Get-WmiObject Win32_Process -Filter "name = 'java.exe'" | Where CommandLine -match "pnc" | Where CommandLine -match "#{$BatchJobLocation}"
			If($a -ne $null){
	...
	powershell_script "stop-#{$AdminWindowServiceName}" do
		code <<-EOH
			$a =  Get-WmiObject Win32_Process -Filter "name = 'java.exe'" | Where CommandLine -match "pnc" | Where CommandLine -match "#{$JbossHome}"
			If($a -ne $null){
				Invoke-WmiMethod -Path $a.__RELPATH -Name terminate 
			}
		EOH
...
if Dir.exists?("#{$BatchJobLocation}\\csc-smart400-batch")	
	...
	execute 'change-folder-backup' do
	  command "ren \"#{$HomeDeployFolder}\\_backups_Admin-Batchjob\\temp\" \"Backup-polisy-%date:/=-%-%time::=-%\""
	end
...
######################### Builds Folder #############################################
	...
	set day=%date:~7,2%
		...
		ren "#{$HomeDeployFolder}\\_builds_Admin-Batchjob\\temp" "Builds-polisy-%month%-%day%-%year%-%time::=-%"
	EOH
	end
